...
public static class ContainerFormMigrationSeeder
{
    private const string MigrationSettingKey = "container_forms_v2_migrated";
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            logger.LogInformation("Container form V2 migration already completed; skipping.");
            return;
        }
        ...
        {
            logger.LogWarning(ex, "Unable to connect to V2 database for container form migration.");
            return;
        }

        var templates = await ReadContainerTemplatesAsync(connection, cancellationToken);
        if (templates.Count == 0)
        {
            logger.LogWarning("No active CONTAINER form templates found in V2.");
            return;
        }
        ...
        foreach (var template in templates)
        {
            ...
            {
                logger.LogWarning(
                    "Skipping V2 container form {V2Id} ({Name}) — no mapped agencies ({Agencies}).",
                    template.Id,
                    template.Name,
            ...
            if (fields.Count == 0)
            {
                logger.LogWarning("Skipping V2 container form {V2Id} ({Name}) — no fields.", template.Id, template.Name);
                continue;
            }
            ...
            logger.LogInformation(
                "Migrated V2 container form {V2Id} ({Name}) with {FieldCount} fields for agencies {Agencies}.",
                template.Id,
                template.Name,
                fields.Count,
                string.Join(", ", template.AgencyCodes));
        ...
        if (migratedCount == 0)
        {
            logger.LogWarning("Container form V2 migration finished without importing any templates.");
            return;
        }
        ...
        logger.LogInformation("Container form V2 migration completed for {Count} template(s).", migratedCount);
    }

    private static async Task UpsertTemplateAsync(
        AgriCheckDbContext db,
        V2ContainerTemplate template,
    ...
    private static async Task<List<V2ContainerTemplate>> ReadContainerTemplatesAsync(
        MySqlConnection connection,
        CancellationToken cancellationToken)
    {
        var templates = new List<V2ContainerTemplate>();
        await using var command = connection.CreateCommand();
        command.CommandText = """
            SELECT id, name, is_active
            FROM form_templates
            WHERE form_type = 'CONTAINER' AND is_active = 1
            ORDER BY id
            """;
        ...
        {
            templates.Add(new V2ContainerTemplate(
                reader.GetInt32(0),
                reader.GetString(1),
                reader.GetBoolean(2),
                Array.Empty<string>()));
    ...
    private sealed class V2ContainerTemplate
    {
        public V2ContainerTemplate(int id, string name, bool isActive, IEnumerable<string> agencyCodes)
        {
            Id = id;
            Name = name;
            IsActive = isActive;
